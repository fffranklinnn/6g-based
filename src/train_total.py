...
def main():
    ...
    raw_state_dim = env_sac._calculate_observation_shape()[0]
    flattened_state_dim = flatten_state(torch.zeros(raw_state_dim)).numel()

    action_dim = env_sac.action_space.numel()
    max_action = 1  # 确认这是否适合你的环境

    sac = SAC(flattened_state_dim, action_dim, max_action, device, env_sac.NUM_SATELLITES, env_sac.NUM_GROUND_USER)

    normalizer = Normalizer.ComplexNormalizer(env_sac.NUM_SATELLITES,env_sac.NUM_GROUND_USER)
    num_episodes = 50
    max_timesteps = 60
    batch_size = 256
    ...
    for episode in range(num_episodes):
        ...
        episode_reward_sac = 0

        for t in range(max_timesteps):
            action = sac.select_action_sac(state)
            action_tensor = torch.tensor(action).to(device)  # 确保 action 是 PyTorch 张量
            action = adjust_action(action_tensor, env_sac.NUM_SATELLITES, env_sac.NUM_GROUND_USER)
            try:
                next_state, reward_sac, done, _ = env_sac.step(action.cpu().numpy())  # 使用转换后的action
            ...
            sac.replay_buffer.add(state.cpu().numpy(), action_tensor.cpu().numpy(), next_state.cpu().numpy(), reward_sac,
                                  not_done)

            state = next_state  # 更新state为下一状态（PyTorch张量）
            episode_reward_sac += reward_sac
            ...
            if done:
                break
        episode_rewards_sac.append(episode_reward_sac.cpu())  # 记录当前 episode 的奖励
        print(f"Episode {episode + 1}, Reward: {episode_reward_sac}")
        ...
        if (episode + 1) % 100 == 0:
            avg_reward = np.mean(
                [env_sac.step(torch.tensor(sac.select_action_sac(flatten_state(env_sac.reset()[0]).to(device))).cpu().numpy())[1]
                 for _ in range(10)])
    ...
    raw_state_dim = env_dqn._calculate_observation_shape()[0]
    flattened_state_dim = flatten_state(torch.zeros(raw_state_dim)).numel()

    action_dim = env_dqn.action_space.numel()
    max_action = 1  # 确认这是否适合你的环境

    dqn = DQN(flattened_state_dim, action_dim, device)

    normalizer = Normalizer.ComplexNormalizer(env_dqn.NUM_SATELLITES, env_dqn.NUM_GROUND_USER)
    num_episodes = 100
    max_timesteps = 60
    batch_size = 256
    ...
    for episode in range(num_episodes):
        ...
        for t in range(max_timesteps):
            ...
            action_tensor = torch.tensor(action).to(device)  # 确保 action 是 PyTorch 张量
            action = adjust_action(action_tensor, env_dqn.NUM_SATELLITES, env_dqn.NUM_GROUND_USER)
            try:
                next_state, reward_dqn, done, _ = env_dqn.step(action.cpu().numpy())  # 使用转换后的action
            except Exception as e:
                print(f"Error during environment step: {e}")
                break
    ...
    plt.figure()
    plt.plot(range(1, num_episodes + 1), episode_rewards_sac, marker='o')    # 有问题
    plt.xlabel('Episode')
    plt.ylabel('Reward')
