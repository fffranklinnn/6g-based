...
class ComplexNormalizer:
    ...
    def normalize(self, state):
        ...
        normalized_state[:self.coverage_space.state_dim] = self.coverage_space.normalize(state[:self.coverage_space.state_dim])
        offset = self.coverage_space.state_dim

        normalized_state[offset:offset + self.previous_access_strategy_space.state_dim] = \
            self.previous_access_strategy_space.normalize(state[offset:offset + self.previous_access_strategy_space.state_dim])
        offset += self.previous_access_strategy_space.state_dim

        normalized_state[offset:offset + self.switch_count_space.state_dim] = \
            self.switch_count_space.normalize(state[offset:offset + self.switch_count_space.state_dim])
        offset += self.switch_count_space.state_dim

        normalized_state[offset:offset + self.elevation_angle_space.state_dim] = \
            self.elevation_angle_space.normalize(state[offset:offset + self.elevation_angle_space.state_dim])
        offset += self.elevation_angle_space.state_dim

        normalized_state[offset:offset + self.altitude_space.state_dim] = \
            self.altitude_space.normalize(state[offset:offset + self.altitude_space.state_dim])
